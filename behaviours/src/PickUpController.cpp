...
bool PickUpController::SetSonarData(float rangeCenter){
  ...
  if (rangeCenter < 0.12 && targetFound) { //Initial range center <.12
...
//Naadir - this was moved up 
  ...
  if (blockDistanceFromCamera < 0.156 && Td < 3.9) { //blockDistanceFromCamera < 0.14 && Td < 3.9
...
Result PickUpController::DoWork() {
  ...
  if (!targetHeld) {
    ...
    float targetDistance = 0.16; //meters intially .15 
...
float raise_time_begin = 2.0;*/
   ...
   /* 
    ...
    {
      lockTarget = true;
      result.pd.cmdVel = 0.14; //Naadir - .15 was changed to .18
      result.pd.cmdAngularError= 0.0;
      timeOut = true;
