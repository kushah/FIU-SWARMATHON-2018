...
void ObstacleController::avoidObstacle() {
    ...
    if (right < 0.4 || center < 0.4 || left < 0.4) { //(right < 0.8 || center < 0.8 || left < 0.8)
      result.type = precisionDriving;
      result.PIDMode = SLOW_PID; // NK- ADDED THIS
...
Result ObstacleController::DoWork() {
  ...
  if (can_set_waypoint) {
    ...
    if Point forward; // NK-Added this
    ...
    /* result.type = waypoint; 
    if Point !foward;
    result.PIDMode = SLOW_PID; //NK- this is work in progress */
  }
